...
describe("Test creation of companion specs from folder", () => {
    test("Try creating from folder", async () => {
        const core = await fs.readFileSync("./tests/resources/00_Opc.Ua.NodeSet2.xml");
        ...
        test("See if target spec is constructed correctly", async () => {
            ...
            const weihenstephan = await fs.readFileSync("./tests/resources/07_Opc.Ua.Weihenstephan.NodeSet2.xml");
